...
class VectorService:
    ...
    def search_with_acl(self, query_vector: List[float], user_roles: List[str], limit: int = 5):
        ...
        # Construct ACL Filter using array_has_any (DataFusion function)
        # We also support a fallback to 'public' if no roles provided.
        roles = user_roles if user_roles else ["public"]
        roles_list_str = ", ".join([f"'{r}'" for r in roles])
        
        # LanceDB 0.10+ supports array_has_any for complex filtering
        acl_filter = f"array_has_any(metadata.roles, [{roles_list_str}])"
            
        try:
            # Vector search with pre-filtering for maximum security
            ...
            print(f"ACL Search Failed (Filter: {acl_filter}): {e}")
            # Fallback to a safer search if array_has_any fails due to versioning
            try:
                 # Alternative approach for older LanceDB/DataFusion
                 fallback_filter = " OR ".join([f"array_has(metadata.roles, '{r}')" for r in roles])
                 results = table.search(query_vector) \
                              .where(fallback_filter, prefilter=True) \
                              .limit(limit) \
                              .to_pandas()
                 return results
            except:
